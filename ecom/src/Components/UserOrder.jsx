...
export default function UserOrder(){
	...
	function handleRemove(id){
		// e.preventDefault();
        // console.log(id);
        axios.get("http://localhost:8080/user/cancel-order/"+id,{
			headers :{
    ...
    const getUserOrders = ()=>{ axios.get("http://localhost:8080/user/user-Order",{
        headers :{
            Authorization:`Bearer ${sessionStorage.getItem('authToken')}`
        }
    }).then((response)=>{
					...
					<table className="table" style={{backgroundColor : "#F4EFE9" , color:"#AD7030"}}>
						...
						<tbody>
                            ...
                            >
								...
								<td>
								<button type="button" class="btn btn-danger" onClick={()=>{handleRemove(c.id)}}>Cancel</button>
								</td>
							</tr>
